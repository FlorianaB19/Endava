...
(
    FirstName, LastName, Email, PhoneNumber, HireDate, Salary, DepartmentID,JobTitle
)
VALUES
...
(
    EmployeeID,WeekStart, WeekEnd,Status,SubmittedDate, ApprovedDate
)
VALUES
...
(
    ProjectName,ClientID,StartDate,EndDate,Budget
)
VALUES
...
(
    EmployeeID,StartDate, EndDate, LeaveType,Status,Reason
)
VALUES
...
(
    EmployeeID, ActionName,IPAddress,Details
)
VALUES
...
/*used to improve project searches by project name(column)*/
...
ON LeaveRequests(StartDate, EmployeeID)
...
/*VIEW
A view is a query saved with a name.*/
...
FROM Employees AS e
INNER JOIN Timesheets AS t --associate each employee with their timesheets
    ON e.EmployeeID = t.EmployeeID
INNER JOIN TimesheetEntries AS te  -- associate each employee with their activity
    ON t.TimesheetID = te.TimesheetID
INNER JOIN Projects AS p   --identify the project for every project
...
/*INDEXED VIEW
an Indexed View stores data physically on disk*/
SET ANSI_NULLS ON; -- how sql compares values
GO

SET QUOTED_IDENTIFIER ON; -- for ex: "" identify object names and '' identify string values
GO

SET ANSI_PADDING ON; --stores trailing spaces in char,varchar,binry,varbinary columns
GO

SET ANSI_WARNINGS ON; -- enables sql to generate warnings for situations like divide by zero
GO

SET CONCAT_NULL_YIELDS_NULL ON; --controls what happens when concatenating strings with NULL
GO

SET ARITHABORT ON; --stop executing a query if an arithmetic error occurs
GO

SET NUMERIC_ROUNDABORT OFF; --what happens when a number loses precision during rounding
GO
...
/*
SQL Server does not support Oracle Materialized Views.
The equivalent object is an Indexed View created using
SCHEMABINDING together with a UNIQUE CLUSTERED INDEX.
*/
...
select * from  dbo.vw_ProjectSummary where ProjectID=7;
...
--employees who havent an submitted timesheets
...
SELECT EntryID,
    JSON_VALUE(AdditionalInfo,'$.workMode') AS WorkMode, -- $ is the root object
    JSON_VALUE(AdditionalInfo,'$.browser') AS Browser,   -- . ... is properties
...
BEGIN
    SET NOCOUNT ON;
    SELECT e.FirstName, e.LastName, SUM(te.HoursWorked) AS TotalHours
    FROM Employees e        --daca angajatul exista dar nu are activitate, nu returneaza nimic
    INNER JOIN Timesheets t
    ON e.EmployeeID=t.EmployeeID
    INNER JOIN TimesheetEntries te
    ON t.TimesheetID=te.TimesheetID
...
END;
GO
EXEC usp_GetEmployeeHours @EmployeeID = 15;
...
storing the employee, action, IP address, and additional information in JSON  ananas format.*/
...
INSERT INTO Timesheets -- insert a new timesheet
(
    EmployeeID,
    WeekStart,
    WeekEnd,
...
SELECT TOP (5) -- the latest five audit logs
       LogID,
       EmployeeID,
       ActionName,
       ActionDate,
...
/*returns the total worked hours for a given employee*/
CREATE FUNCTION dbo.fn_GetEmployeeTotalHours  --scalar function
(
    @EmployeeID INT
)
RETURNS DECIMAL(10,2) -- the function returns just a 
AS --the implementation of the function starts here
BEGIN
    DECLARE @TotalHours DECIMAL(10,2); 
    SELECT
        @TotalHours = ISNULL(SUM(te.HoursWorked), 0)
    FROM Timesheets t
    INNER JOIN TimesheetEntries te
        ON t.TimesheetID = te.TimesheetID
...
END;
GO
SELECT dbo.fn_GetEmployeeTotalHours(5);


/*display all active employees earning more than 5000*/
SELECT EmployeeID,FirstName,LastName,Salary
FROM dbo.Employees 
WHERE Salary >5000 AND IsActive = 1
ORDER BY Salary DESC;



/*classifies employees based on salary*/
SELECT FirstName, LastName, Salary,
    CASE WHEN Salary < 8000 THEN 'Junior'
...
BEGIN
    ...
    SELECT
        @FullName =
        FirstName + ' ' + LastName --retunr the complete name 
    FROM Employees
    WHERE EmployeeID = @EmployeeID;
    RETURN @FullName;
END;
...
/*TABLE VALUED GUNCTION*/
CREATE FUNCTION dbo.fn_DepartmentEmployees 
( 
    @DepartmentID INT --the function accepts one input parameter
)
RETURNS TABLE  /*will return an entire table*/
AS
RETURN
(
    SELECT EmployeeID, FirstName,LastName,Salary
...
SELECT *
FROM dbo.fn_DepartmentEmployees(20);
...
END TRY
BEGIN CATCH
    PRINT ERROR_MESSAGE(); --if salary=-100 ten print error message bcs check(salary) > 0
END CATCH
END;
GO
...
/*
Displays employee names using a cursor.
*/
DECLARE @FirstName NVARCHAR(100);  --A variable named @FirstName is created
DECLARE EmployeeCursor CURSOR
FOR
SELECT FirstName
FROM Employees;
OPEN EmployeeCursor;
FETCH NEXT --reads the first row
FROM EmployeeCursor
INTO @FirstName;
WHILE @@FETCH_STATUS = 0 --check if the fetch was ok, value: 0, -1,-2
BEGIN
    PRINT @FirstName; 
    FETCH NEXT  --fetc the next row
    FROM EmployeeCursor
    INTO @FirstName;
END;
...
/*
Updates the salary of employee 15.
*/
EXEC usp_UpdateSalary
     @EmployeeID = 15,
     @Salary = 9000;
...
CREATE FUNCTION dbo.fn_RemainingProjectBudget
(
    @ProjectID INT
)
...
BEGIN
    ...
    SELECT
        @RemainingBudget =
        p.Budget - ISNULL(SUM(te.HoursWorked * 50),0)
    FROM Projects p
    LEFT JOIN TimesheetEntries te
        ON p.ProjectID = te.ProjectID
...
SELECT ProjectName, dbo.fn_RemainingProjectBudget(ProjectID)
FROM Projects;
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        IF NOT EXISTS --verfy if timesheet exists
        (
            SELECT 1
            FROM Timesheets
        ...
        (
            ...
            FROM Timesheets
            WHERE TimesheetID = @TimesheetID
            AND Status <> 'Draft' --bcs if timesheet is submtted or approved it cant be modify
        )
        BEGIN
            PRINT 'Only draft timesheets can be submitted.';
            RETURN;
        END;
...
EXEC usp_SubmitTimesheet @TimesheetID = 9;
SELECT
    TimesheetID,
    Status,
    SubmittedDate
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            PRINT 'Only submitted timesheets can be approved.';
            RETURN;
        END;
        UPDATE Timesheets  --approved: Submitted -> Approved
        SET
            Status = 'Approved',
